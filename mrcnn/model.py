...
Mask R-CNN
----------------------------------------------------
This file contains the main Mask R-CNN model implementation, adapted for a
specialized dental application. It builds upon the foundational work of
Matterport, Inc. and the TensorFlow 2.x updates from the z-mahmud22 fork.

This version includes several key customizations by Arash Nedaei:
-   **Modified Detection Layer**: The `refine_detections_graph` function has been
    edited to allow for separate control over the maximum number of detected
    instances per class and the total number of detections in an image. This is
    critical for the dental use case where only one of each tooth number should
    be detected.
-   **TensorFlow 2.x Compatibility**: Includes a `tf.keras.utils.Sequence` based
    `DataGenerator` for efficient, multiprocessing-safe data loading in TF2.
-   **Dynamic Learning Rate**: A custom learning rate scheduler has been added to
    the `train` method to allow for more flexible training schedules.
...
tf.compat.v1.disable_eager_execution() # right after `import tensorflow as tf` https://stackoverflow.com/questions/58137677/keras-model-training-memory-leak #Arash
...
session = InteractiveSession(config=config)#session = tf.compat.v1.Session(config=config)#

# Requires TensorFlow 2.0+
from distutils.version import LooseVersion
assert LooseVersion(tf.__version__) >= LooseVersion("2.0")

from keras.callbacks import LearningRateScheduler #Arash

# tf.compat.v1.disable_eager_execution() #Arash
...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    # # Gather the results after class-wise NMS
    # final_rois = tf.gather(refined_rois, keep)
    # final_scores = tf.gather(class_scores, keep)
    # final_class_ids = tf.gather(class_ids, keep)

    # # Apply NMS on all detections, regardless of class

    # final_nms_keep = tf.image.non_max_suppression(
    #     boxes=final_rois,
    #     scores=final_scores,
    #     max_output_size=config.DETECTION_MAX_INSTANCES_ALL_CLASSES,  # Adjust as needed
    #     iou_threshold=config.DETECTION_NMS_THRESHOLD_ALL_CLASSES  # Adjust IoU threshold as needed for all classes
    # )

    # # Gather the final indices after the global NMS
    # keep = tf.gather(keep, final_nms_keep)



    # ######################################################################
    # # Cross-class NMS 26.09.2025
    # ######################################################################
    # Gather the results after class-wise NMS
    final_rois = tf.gather(refined_rois, keep)
    final_scores = tf.gather(class_scores, keep)
    ...
    # ----- SAFER GLOBAL (CROSS-CLASS) NMS -----
    # Ensure correct dtype
    final_rois = tf.cast(final_rois, tf.float32)
    final_scores = tf.cast(final_scores, tf.float32)

    # Convert normalized boxes to pixel coordinates before NMS to improve numeric stability.
    # Use config.IMAGE_SHAPE which is static Python values (height, width, channels).
    img_h = tf.constant(config.IMAGE_SHAPE[0], dtype=tf.float32)
    img_w = tf.constant(config.IMAGE_SHAPE[1], dtype=tf.float32)
    scale = tf.stack([img_h, img_w, img_h, img_w])
    final_rois_px = final_rois * scale

    # Make sure threshold is a float32 tensor
    iou_thr = tf.constant(config.DETECTION_NMS_THRESHOLD_ALL_CLASSES, dtype=tf.float32)

    # Limit max_output_size to the number of candidate boxes (avoid oversized requests)
    max_out = tf.minimum(tf.shape(input=final_scores)[0],
                        tf.constant(config.DETECTION_MAX_INSTANCES_ALL_CLASSES, dtype=tf.int32))

    # Run global (all-classes) NMS, greedy by score (get candidate indices)
    final_nms_keep = tf.image.non_max_suppression(
        boxes=final_rois_px,
        scores=final_scores,
    ...
    # Map the selected indices back to indices in the original `refined_rois` space
    keep = tf.gather(keep, final_nms_keep)
    # final_nms_keep are indices into final_rois_px sorted by selection order (higher scores first)
    # Now compute pairwise IoF among the selected boxes and suppress any box that is
    # mostly contained in a higher-scoring box. This handles the "small box inside big box"
    # failure mode of IoU-based NMS.
    sel = final_nms_keep
    sel_boxes = tf.gather(final_rois_px, sel)  # [M,4] in pixel coords

    # If no boxes selected, keep empty
    def _empty_keep():
        return tf.constant([], dtype=tf.int32)

    def _compute_keep():
        # split coords
        y1, x1, y2, x2 = tf.split(sel_boxes, 4, axis=1)  # each [M,1]
        h = y2 - y1
        w = x2 - x1
        areas = tf.reshape((h * w)[:, 0], [-1])  # [M]

        # pairwise intersection
        y1_max = tf.maximum(y1, tf.transpose(y1))
        x1_max = tf.maximum(x1, tf.transpose(x1))
        y2_min = tf.minimum(y2, tf.transpose(y2))
        x2_min = tf.minimum(x2, tf.transpose(x2))
        inter_h = tf.maximum(0.0, y2_min - y1_max)
        inter_w = tf.maximum(0.0, x2_min - x1_max)
        inter = inter_h * inter_w  # [M, M]

        # IoF: intersection over min(area_i, area_j)
        areas_row = tf.reshape(areas, [-1, 1])
        areas_col = tf.reshape(areas, [1, -1])
        min_area = tf.maximum(tf.minimum(areas_row, areas_col), 1.0)  # avoid div by zero
        iof = inter / min_area  # [M, M]

        # Build "earlier" mask: True where row_index < col_index (i < j)
        M = tf.shape(sel)[0]
        idx = tf.range(M)
        r = tf.reshape(idx, [-1, 1])
        c = tf.reshape(idx, [1, -1])
        earlier = tf.less(r, c)  # [M, M]

        # For each column j (candidate box), check if any earlier i has iof(i,j) > threshold
        suppress_by_iof = tf.reduce_any(tf.logical_and(iof > iou_thr, earlier), axis=0)

        # keep_selected_mask are booleans for positions 0..M-1 that survive
        keep_mask = tf.logical_not(suppress_by_iof)

        # IMPORTANT: use positions (0..M-1), not sel values, to index the current `keep` tensor
        positions = tf.range(M, dtype=tf.int32)
        keep_positions = tf.boolean_mask(positions, keep_mask)          # positions into keep (0..M-1)
        # selected indices into final_rois_px if you need them:
        # final_keep_sel = tf.gather(sel, keep_positions)

        return keep_positions

    final_keep_pos = tf.cond(tf.equal(tf.shape(input=sel)[0], 0),
                              _empty_keep, _compute_keep)

    # Map the selected positions to the original `keep` indices
    # (keep currently == tf.gather(original_keep, final_nms_keep))
    keep = tf.gather(keep, final_keep_pos)
    ...
    # Keep top detections
    roi_count = config.DETECTION_MAX_INSTANCES_ALL_CLASSES #config.DETECTION_MAX_INSTANCES #Arash
    ...
    # Pad with zeros if detections < DETECTION_MAX_INSTANCES_ALL_CLASSES
    gap = config.DETECTION_MAX_INSTANCES_ALL_CLASSES - tf.shape(input=detections)[0] #config.DETECTION_MAX_INSTANCES - tf.shape(input=detections)[0] #Arash
...
class DetectionLayer(KL.Layer):
    ...
    def call(self, inputs):
        ...
        # print("detection number all: ", self.config.DETECTION_MAX_INSTANCES_ALL_CLASSES)
        return tf.reshape(
            detections_batch,
            [self.config.BATCH_SIZE, self.config.DETECTION_MAX_INSTANCES_ALL_CLASSES, 6]) #self.config.DETECTION_MAX_INSTANCES, 6]) #Arash

    def compute_output_shape(self, input_shape):
        return (None, self.config.DETECTION_MAX_INSTANCES_ALL_CLASSES, 6)#(None, self.config.DETECTION_MAX_INSTANCES, 6)#Arash
...
def load_image_gt(dataset, config, image_id, augmentation=None):
    ...
    #print("data generator, __getitem: image_id {}, shape gt_masks {}, image original shape {} ".format(image_id, mask.shape, original_shape))#Arash
...
def build_rpn_targets( anchors, gt_class_ids, gt_boxes, config):#(image_shape, anchors, gt_class_ids, gt_boxes, config):#Arash
...
# class DataGenerator(KU.Sequence):#Arash
...
#         self.config = config
#         #print("image ids in dataset: ", len(self.image_ids))#Arash
...
#         #print("end of generator run")#Arash
...
class DataGenerator(KU.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        # print("image ids: ", image_ids)#Arash
...
class MaskRCNN(object):
    ...
    def compile(self, learning_rate, momentum):
        ...
        #optimizer = keras.optimizers.SGD(
            #lr=learning_rate, momentum=momentum,#Arash 14092024
        ...
        for name in loss_names:
            ...
            #print("compile > loss: ", loss)#Arash
            self.keras_model.add_loss(loss)

        # Add L2 Regularization
        # Skip gamma and beta weights of batch normalization layers.
        #self.keras_model.trainable_weights#Arash
        reg_losses = [
    ...
    def set_trainable(self, layer_regex, keras_model=None, indent=0, verbose=1):
        ...
        for layer in layers:
            ...
            if layer.__class__.__name__ == 'Model':
                #print("In model: ", layer.name)#Arash
                self.set_trainable(
    ...
    ###########
    #Arash  04.10.24 editted 18.12.24
    ###########
    # def scheduler(self, epoch, lr):
    #     lr = lr
    #     if epoch <= self.config.LEARNING_RATE_EPOCH_STEPS[0]:
    #         lr = self.config.LEARNING_RATE_LIST[0]
    #         # K.set_value(self.keras_model.optimizer.lr, self.config.LEARNING_RATE_LIST[0])
    #     elif epoch <= self.config.LEARNING_RATE_EPOCH_STEPS[1]:
    #         lr = self.config.LEARNING_RATE_LIST[1]
    #         # K.set_value(self.keras_model.optimizer.lr, self.config.LEARNING_RATE_LIST[1])
    #     elif epoch <= self.config.LEARNING_RATE_EPOCH_STEPS[2]:
    #         lr = self.config.LEARNING_RATE_LIST[2]
    #         # K.set_value(self.keras_model.optimizer.lr, self.config.LEARNING_RATE_LIST[2])
    #     else:
    #         lr = self.config.LEARNING_RATE_LIST[3]
    #         # K.set_value(self.keras_model.optimizer.lr, self.config.LEARNING_RATE_LIST[3])
    #     # return K.get_value(self.keras_model.optimizer.learning_rate)
    #     return lr
    
    ###########
    #Arash  02.01.25
    ###########
    def scheduler(self, epoch, lr):#, config):
        new_lr = self.config.LEARNING_RATE_LIST[-1]
        # lr = config.LEARNING_RATE_LIST[-1]
        for i, step in enumerate(self.config.LEARNING_RATE_EPOCH_STEPS):
            if epoch < step:
                new_lr = self.config.LEARNING_RATE_LIST[i]
                break
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        callbacks = [
            ...
            keras.callbacks.ModelCheckpoint(self.checkpoint_path, # Arashdisabled for tunner 240125
        ...
        # Train
        log("\nStarting at epoch {}. LR={}\n".format(self.epoch, self.scheduler(self.epoch, learning_rate)))
        log("Checkpoint Path: {}".format(self.checkpoint_path))
        self.set_trainable(layers)
        #self.compile(learning_rate(self.scheduler), self.config.LEARNING_MOMENTUM)
        self.compile(self.scheduler(self.epoch, learning_rate), self.config.LEARNING_MOMENTUM)
